  ...
  void * untyped_ros_message)
{
  ...
  // Field name: scales
  {
    ...
    if (!geometry_msgs__msg__Vector3__Sequence__init(&ros_message->scales, size)) {
      return "failed to create array for field 'scales'";
    }
    auto array_ptr = ros_message->scales.data;
    for (size_t i = 0; i < size; ++i) {
      if (!callbacks->cdr_deserialize(
...
static size_t _IsaacPose_Request__max_serialized_size(bool & full_bounded)
{
  return max_serialized_size_isaac_ros2_messages__srv__IsaacPose_Request(
    full_bounded, 0);
}
...
static size_t _IsaacPose_Response__max_serialized_size(bool & full_bounded)
{
  return max_serialized_size_isaac_ros2_messages__srv__IsaacPose_Response(
    full_bounded, 0);
}
